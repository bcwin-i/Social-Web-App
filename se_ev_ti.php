...
if(mysqli_num_rows($result) < 1)
{
	$output = '<table style="margin: 2px; margin-left:0px; margin-top:5px; width: 100%;  margin-bottom: 0px;">
				<tr><td ><span style="color: gray; font-size: 14px; font-family: Calibri; text-align: center; width:100% ">Unknown event title!</span></td></tr></table>';
	echo $output;
	exit();
}
...
while($row = mysqli_fetch_assoc($result)){
	$output .= '<tr><td style="border-bottom: 0.5px solid #F0F0F0; height: 25px;"><span class="event_link" id='.$row['id'].' style="color: #B81D1A; opacity: 0.5; cursor:pointer; overflow:hidden; font-size: 14px; font-family: Calibri; text-align: center; width:100% ">'.$row['event_title'].'</span></td></tr>';
}
